...
import { Info, Loader2, Lock, LogIn, Mail, Shield, X } from "lucide-react";
import { userLoginSchema, type LoginInputState } from "../schema/userSchema";

// <== ADMIN LOGIN COMPONENT ==>
const AdminLogin = () => {
  // USE TITLE HOOK
  useTitle("Foodies - Admin Login");
  // NAVIGATION
  const navigate = useNavigate();
  // TEMPORARY ADMIN LOGIN LOADER VARIABLE
  ...
  const changeEventHandler = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    // SETTING INPUT VALUE FOR THE FIELDS
  ...
  // ADMIN LOGIN HANDLER
  const adminLoginHandler = async (e: FormEvent) => {
    // PREVENTING DEFAULT
    e.preventDefault();
      ...
      <section className="w-screen h-screen flex items-center justify-center overflow-hidden">
        ...
        >
          ...
          <ErrorModal
            ...
            errors={modalErrors}
            title="Admin Login Validation Failed!"
            description="Please review your admin credentials carefully!"
          />
        </motion.section>
      </section>
    </>
